      ...
      if (fbUser) {
        ...
        if (isNativeApp()) {
          ...
          getNativeFcmToken()
            ...
            .catch(() => {});
        } else if (listenerRole === 'helper' || listenerRole === 'store') {
          // Initialize FCM push token for this device (async, non-blocking).
          // Only helpers and stores are prompted on load.
          ...
          // Customers are never prompted on load — init only if already granted.
          initFcmMessaging(fbUser.uid).catch(() => {});
        }
      } else {
        setUser(null);
