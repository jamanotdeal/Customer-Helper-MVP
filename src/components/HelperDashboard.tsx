...
export const HelperDashboard: React.FC<HelperDashboardProps> = ({
  ...
  const handleViewOrderDetails = (orderId: string) => {
    const freshOrder = fallbackStore.orders.get(orderId);
    if (freshOrder && freshOrder.helperId && freshOrder.helperId !== user?.uid && freshOrder.customerId !== user?.uid && !user?.isAdmin) {
      showAlert(
        'অর্ডারটি ইতিমধ্যে গৃহীত হয়েছে',
        'দুঃখিত, এই অর্ডারটি ইতিমধ্যে অন্য একজন হেলপার গ্রহণ করেছেন। আপনি আর এই অর্ডারের বিবরণ দেখতে পারবেন না।',
        'warning'
      ...
      if (targetOrder.helperId && targetOrder.helperId !== user?.uid && targetOrder.customerId !== user?.uid && !user?.isAdmin) {
        return (
          <div className="flex flex-col items-center justify-center min-h-[60vh] space-y-4 p-8 text-center bg-white rounded-3xl border border-gray-100 shadow-sm my-4">
            ...
            <h3 className="font-extrabold text-gray-800 text-base">অর্ডারটি ইতিমধ্যে গৃহীত হয়েছে</h3>
      ...
      <div className="flex space-x-2 bg-gray-100 p-1.5 rounded-2xl">
        ...
        >
          ...
          {newOrderIds.size > 0 && activeTab !== 'NEW' && (
            <span className="absolute -top-1 -right-1 min-w-[16px] h-[16px] px-1 rounded-full bg-red-500 text-white text-[9px] font-extrabold flex items-center justify-center animate-bounce shadow-md">
              {newOrderIds.size}
            </span>
          )}
        </button>
        ...
        >
          ...
          <span>Running{filteredActiveOrders.length > 0 && ` (${filteredActiveOrders.length})`}</span>
          {unviewedActiveCount > 0 && activeTab !== 'ACTIVE' && (
            <span className="absolute -top-1 -right-1 min-w-[16px] h-[16px] px-1 rounded-full bg-blue-500 text-white text-[9px] font-extrabold flex items-center justify-center animate-bounce shadow-md">
              {unviewedActiveCount}
            </span>
          )}
        </button>
