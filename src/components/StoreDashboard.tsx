            ...
            <div className="bg-white rounded-3xl border border-gray-100 p-5 shadow-soft space-y-4">
              ...
              <div className="relative flex items-center justify-between w-full px-2 pt-2 pb-4">
                ...
                <div 
                  ...
                  style={{ width: `calc(${currentShopOrder.status !== 'PENDING' ? (shopOrderSteps.findIndex((s) => s.status === currentShopOrder.status) / (shopOrderSteps.length - 1)) * 100 : 0}% - 8px)` }}
                />
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className={`text-[10px] font-extrabold mt-2 text-center leading-tight ${
                        state === 'CURRENT' ? 'text-emerald-700' : state === 'COMPLETED' ? 'text-gray-800' : 'text-gray-300'
                      }`}>
                        {step.label}
                      </span>
                    </div>
                  ...
                  {currentShopOrder.status === 'PENDING' ? (
                    <div className="space-y-2">
                      ...
                      <button
                        ...
                        className="w-full py-2.5 bg-red-50 hover:bg-red-100 text-red-700 rounded-2xl text-xs font-bold border border-red-100 transition-all active:scale-95 text-center"
                      >
                        Reject
                      </button>
                    </div>
    ...
    <div className="space-y-5 pb-24 animate-in fade-in duration-200">
      ...
      <div className="flex space-x-1.5 bg-gray-100 p-1.5 rounded-2xl">
        ...
        >
          ...
          <span>Incoming Orders ({storeShopOrders.length})</span>
        </button>
            ...
            {(['NEW', 'RUNNING', 'COMPLETED'] as const).map((tab) => {
              ...
              return (
                <button
                  ...
                  className={`flex-1 py-2 rounded-lg text-xs font-bold transition-all ${tabStyle}`}
                >
                  {tab === 'NEW' ? 'New' : tab === 'RUNNING' ? 'Running' : 'Completed'}{' '}
                  <span className={isRunningWithCount ? 'animate-blink font-black px-1.5 py-0.5 rounded bg-red-600 text-white text-[10px]' : ''}>
                    ({count})
                  </span>
                </button>
              );
                ...
                {filteredShopOrders.slice(0, storeVisibleCount).map((so) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {/* Actions: Pricing removed, wide details button */}
                      <div className="flex gap-2 pt-2">
                        ...
                        <button
                          ...
                          className={`${(so.status === 'PENDING' && !isCanceled && !isDelivered) ? 'flex-1' : 'w-full'} py-2.5 bg-gray-100 hover:bg-gray-200 text-gray-800 rounded-xl text-xs font-bold transition-all border border-gray-200 text-center`}
                        >
                          Details
                        </button>
                      </div>
